  ...
  return (
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-3xl p-6 max-w-sm w-full shadow-2xl text-center">
        ...
        <h3 className="text-lg font-bold mb-1">Out of Energy!</h3>
        <p className="text-gray-500 text-sm mb-4">Wait for regeneration or come back later!</p>
        <div className="bg-gradient-to-br from-purple-100 to-pink-100 rounded-2xl p-4 mb-4">
          <div className="text-2xl">⏰</div>
          <div className="font-bold text-sm">Energy regenerates: 1/hour</div>
        </div>
    ...
    <div className="bg-white rounded-3xl p-6 shadow-lg">
      <div className="text-center mb-4">
        ...
        <p className="text-gray-500 text-sm">
          {hasVoted ? 'Thanks for voting!' : 'Pick your side! (1 vote per battle)'}
        </p>
      </div>
      ...
      <div className="bg-gray-100 rounded-full h-3 overflow-hidden">
        <div className="flex h-full">
          ...
          />
          <div
            className="bg-gradient-to-r from-pink-400 to-orange-500"
  ...
  return (
    <div className="bg-white rounded-3xl p-5 shadow-lg">
      ...
      <div className="relative h-56 bg-gradient-to-b from-indigo-900 via-blue-800 to-green-700 rounded-2xl overflow-hidden mb-3">
        ...
        <div
          className={`absolute left-1/2 text-xl transition-all duration-500 ease-out ${
            atMoon || atEarth ? 'animate-bounce' : ''
          }`}
          style={{
  ...
  if (memeState.meme_found) {
    return (
      <div className="bg-white rounded-3xl p-6 shadow-lg text-center">
        ...
        <p className="text-sm text-gray-400">Found in {memeState.total_reveals} attempts!</p>
      </div>
    );
  }
      ...
      {filtered.length === 0 ? (
        ...
        <div className="space-y-1 max-h-28 overflow-y-auto">
          {filtered.slice(0, 5).map((w, i) => (
            <div
              key={w.id}
              className={`flex items-center gap-2 p-1.5 rounded-lg ${i === 0 ? 'bg-yellow-50' : 'bg-gray-50'}`}
            >
...
export default function IdiotGames() {
  // Initialize player from localStorage synchronously on first render
  const [player, setPlayer] = useState<Player | null>(() => {
    if (typeof window === 'undefined') return null;
    const savedPlayer = localStorage.getItem(STORAGE_KEY + '-data');
    if (savedPlayer) {
      try {
  ...
  const [win, setWin] = useState<{ type: 'moon' | 'earth' | 'meme'; attempts?: number } | null>(
    null
  );
  ...
  // Fetch initial game state when player is loaded
  useEffect(() => {
    if (player) {
  ...
  const handleVote = async (side: 'left' | 'right') => {
    if (!player || !gameState?.teamVoteState || actionLoading) return;
    ...
    try {
      ...
      if (data.error) {
        ...
        setPlayer(data.player);
        localStorage.setItem(STORAGE_KEY + '-data', JSON.stringify(data.player));
        setPlayerVotes([...playerVotes, { battleId: gameState.teamVoteState.battle_id, side }]);
  ...
  const handleMove = async (direction: 'moon' | 'earth') => {
    if (!player || !gameState?.moonBallState || actionLoading) return;
    ...
    try {
      ...
      if (data.error) {
        ...
        setGameState({
          ...
          hallOfFame: data.hallOfFame || gameState.hallOfFame,
        });
        if (data.winner) {
  ...
  const handleReveal = async (cellIndex: number) => {
    if (!player || !gameState?.memeGameState || actionLoading) return;
  ...
  const hasVotedCurrent =
    gameState?.teamVoteState && playerVotes.some((v) => v.battleId === gameState.teamVoteState.battle_id);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-50 via-pink-50 to-orange-50">
      {showAd && <AdModal onComplete={() => setShowAd(false)} />}
        ...
        <div className="grid md:grid-cols-3 gap-4">
          ...
          <div className="space-y-4">
            ...
            <div className="bg-gradient-to-br from-purple-100 to-pink-100 rounded-2xl p-4 text-center">
              ...
              <p className="text-xs text-gray-400 mt-2">{player.energy} free plays left</p>
            </div>
            <div className="bg-white rounded-2xl p-4 shadow">
              <div className="text-center">
